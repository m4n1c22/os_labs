...
static struct semaphore excluser;
static struct semaphore n_reader;
static struct semaphore n_writer;
...
static ssize_t fifo_module_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
	...
	*/
	...
	if (down_interruptible(&n_writer)){
		printk(KERN_ALERT "writer access failed");
		return -ERESTARTSYS;
	}
	else {
		if (down_interruptible(&excluser)){
			printk(KERN_ALERT "exclusive access failed");
			up(&n_writer);
			return -ERESTARTSYS;
		}
	else {
...
static int __init fifo_module_init(void)
{
	...
	sema_init(&excluser,1);
	sema_init(&n_reader,0);
	sema_init(&n_writer,MAX_ITEMS);
